...
class LogInterpreter:
    ...
    def getActivities(self, as_df=False):
        ...
        columns = ["bid_price_1", "bid_volume_1", "bid_price_2", "bid_volume_2", "bid_price_3", "bid_volume_3",
        ...
        for line in self.file:
            ...
            activities[(product, int(timestamp))] = dict(
                zip(columns, [*map(lambda x: None if x == "" else float(x), prices)]))
        if as_df:
...
if __name__ == "__main__":
    ...
    sandbox_logs_df = log_interpreter.getSandboxLogs(True)
    # sandbox_logs_df.to_csv('sandbox_logs.csv', index=False)