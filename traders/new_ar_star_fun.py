...
class Trader:
    ...
    def compute_orders_amethysts(self, product, order_depth: OrderDepth, acc_bid, acc_ask):
        orders: list[Order] = []
        ...
        if cpos < pos_lim:
            # Calculate total positions to distribute across bid levels
            total_bid_positions = (pos_lim - cpos)
            
            # Distribute positions evenly across bid levels
            bid_level_1_qty = total_bid_positions // 2
            bid_level_2_qty = total_bid_positions - bid_level_1_qty
            
            orders += [Order(product, price_bid, bid_level_1_qty)]
            orders += [Order(product, price_bid + 1, bid_level_2_qty)]
        ...
        if cpos > -pos_lim:
            # Calculate total positions to distribute across ask levels
            total_ask_positions = -(pos_lim - cpos)
            
            # Distribute positions evenly across ask levels
            ask_level_1_qty = total_ask_positions // 2
            ask_level_2_qty = total_ask_positions - ask_level_1_qty
            
            orders += [Order(product, price_ask, ask_level_1_qty)]
            orders += [Order(product, price_ask - 1, ask_level_2_qty)]
