...
class Trader:
    ...
    sf_bid_params = [15.227814, 0.453990, 0.542945]
    sf_ask_params = [15.227814, 0.453990, 0.542945]
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        if len(self.sf_cache) == (len(self.sf_bid_params) - 1):
            self.sf_cache.pop(0)
        ...
        if len(self.sf_cache) == (len(self.sf_mid_params) - 1):
            s_mid_price = int((np.array(self.sf_cache) * np.array(self.sf_mid_params[1:])).sum() + self.sf_mid_params[0])

            starfruit_lb = s_mid_price-1
            starfruit_ub = s_mid_price+1
