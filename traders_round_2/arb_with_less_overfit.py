...
class Trader:
    ...
    def get_orchid_prices(self, added_val, top_ask, top_buy):
        ...
        if top_ask - top_buy > 4:
            ...
            if added_val < 2.168099856680999:
                ask_pr_1 = mid_price
                ask_pr_2 = mid_price
            else:
                ask_pr_1 = mid_price-1
                ask_pr_2 = mid_price-1

        else:
            ask_pr_1 = top_ask-1
            ask_pr_2 = top_ask-1

        return ask_pr_1, ask_pr_2
    ...
    def computer_orchids_orders(self, product, order_depth, observation):
        ...
        sell_pr_1, sell_pr_2 = self.get_orchid_prices(-fee-imp, top_ask, top_buy)

        arb_val = top_buy-ap-fee-imp
        ...
        if apos < 0:
            if arb_val-1 > 0:
                ...
                bid_order = max(int(self.orch_ask)-1, top_buy+1)
                orders.append(Order(product, bid_order, -pos_lim-apos))
            

        sell_vol_1 = int((-pos_lim-apos)*0.5)
        sell_vol_2 = -pos_lim-apos-sell_vol_1
        
        self.orch_ask = (sell_pr_1+sell_pr_2)/2
