        ...
        if self.straw_pct < 0.3435565437395282-0.0026946263295697507:
            ...
            if self.choco_pct < 0.3435565437395282+0.0026946263295697507*0.75 and self.choco_pct > 0.3435565437395282-0.0026946263295697507*0.75:
                if self.position['STRAWBERRIES'] > 0:
                    orders['STRAWBERRIES'].append(Order('STRAWBERRIES', straw_buy, max(-1, -self.position['STRAWBERRIES'])))
                elif self.position['STRAWBERRIES'] < 0:
                    orders['STRAWBERRIES'].append(Order('STRAWBERRIES', straw_sell, min(1, -self.position['STRAWBERRIES'])))
        
        if self.roses_pct < 0.2062692721893589-0.0011968583761259347*0.75:
            orders['ROSES'].append(Order('ROSES', roses_sell, min(self.POSITION_LIMIT['ROSES']-self.position['ROSES'], 1)))
        elif self.roses_pct > 0.2062692721893589+0.0011968583761259347*0.75:
            orders['ROSES'].append(Order('ROSES', roses_buy, max(-1, -self.POSITION_LIMIT['ROSES']-self.position['ROSES'])))
        else:
            if self.roses_pct < 0.2062692721893589+0.0011968583761259347*0.25 and self.choco_pct > 0.2062692721893589-0.0011968583761259347*0.25:
                if self.position['ROSES'] > 0:
                    orders['ROSES'].append(Order('ROSES', roses_buy, max(-1, -self.position['ROSES'])))
                elif self.position['ROSES'] < 0:
                    orders['ROSES'].append(Order('ROSES', roses_sell, min(1, -self.position['ROSES'])))
