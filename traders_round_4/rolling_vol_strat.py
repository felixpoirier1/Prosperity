...
class Trader:
    ...
    def compute_coconuts_orders(self, coconuts_depth: OrderDepth, coconuts_coupon_depth: OrderDepth) -> Tuple[list[Order], list[Order]]:
        ...
        self.vol_lst.append(mid_coco)
        if len(self.vol_lst) == 3:
            pct_changes = []
            for i in range(1, len(self.vol_lst)):
                pct_change = ((self.vol_lst[i] - self.vol_lst[i - 1]) / self.vol_lst[i - 1]) * 100
                pct_changes.append(pct_change)

            imp_vol = np.std(pct_changes)
            ...
            logger.print(f'imp val: {imp_call_val}')
            logger.print(f'bid, mid, ask: {top_bid_coup}, {mid_coco_coup}, {top_ask_coup}')

            if imp_call_val < mid_coco_coup and self.position['COCONUT_COUPON']-2 >= -600:
                orders_coco.append(Order('COCONUT', top_ask_coco, 1))
                orders_coupon.append(Order('COCONUT_COUPON', top_bid_coup, -2))
            elif imp_call_val > mid_coco_coup and self.position['COCONUT_COUPON']+2 <= 600:
