...
class Trader:
    ...
    def _compute_etf_orders(self, order_depths: Dict[Symbol, OrderDepth], synth_bid: int, synth_ask:int, own_trades, market_trades) -> dict[Symbol, list[Order]]:
        ...
        try:
            if market_trades['ROSES'][0].buyer == 'Rhianna' or market_trades['ROSES'][0].buyer == 'Vinnie':
                orders['ROSES'].append(Order('ROSES', roses_sell, self.POSITION_LIMIT['ROSES']-self.position['ROSES']))
            elif market_trades['ROSES'][0].seller == 'Rhianna' or market_trades['ROSES'][0].seller == 'Vinnie':
                orders['ROSES'].append(Order('ROSES', roses_buy, -self.POSITION_LIMIT['ROSES']-self.position['ROSES']))
        except Exception as e:
            pass

        try:
            if market_trades['CHOCOLATE'][0].buyer == 'Vinnie':
                orders['CHOCOLATE'].append(Order('CHOCOLATE', roses_sell, self.POSITION_LIMIT['CHOCOLATE']-self.position['CHOCOLATE']))
            elif market_trades['CHOCOLATE'][0].seller == 'Vinnie':
                orders['CHOCOLATE'].append(Order('CHOCOLATE', roses_buy, -self.POSITION_LIMIT['CHOCOLATE']-self.position['CHOCOLATE']))
        except Exception as e:
            pass

        try:
            if market_trades['STRAWBERRIES'][0].buyer == 'Vinnie':
                orders['STRAWBERRIES'].append(Order('STRAWBERRIES', roses_sell, self.POSITION_LIMIT['STRAWBERRIES']-self.position['STRAWBERRIES']))
            elif market_trades['STRAWBERRIES'][0].seller == 'Vinnie':
                orders['STRAWBERRIES'].append(Order('STRAWBERRIES', roses_buy, -self.POSITION_LIMIT['STRAWBERRIES']-self.position['STRAWBERRIES']))
        except Exception as e:
            pass
