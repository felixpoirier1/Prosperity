...
class Trader:
    ...
    def _compute_etf_orders(self, order_depths: Dict[Symbol, OrderDepth]) -> dict[Symbol, list[Order]]:
        ...
        if self.side == 'undervalued':
            orders['GIFT_BASKET'].append(Order('GIFT_BASKET', gift_sell, min(1, 58-self.position['GIFT_BASKET'])))
            orders['CHOCOLATE'].append(Order('CHOCOLATE', choco_buy, max(-4, -232-self.position['CHOCOLATE'])))
            orders['ROSES'].append(Order('ROSES', roses_buy, max(-1, -58-self.position['ROSES'])))
            orders['STRAWBERRIES'].append(Order('STRAWBERRIES', straw_buy, max(-6, -348-self.position['STRAWBERRIES'])))
        elif self.side == 'overvalued':
            orders['GIFT_BASKET'].append(Order('GIFT_BASKET', gift_buy, max(-1, -58-self.position['GIFT_BASKET'])))
            orders['CHOCOLATE'].append(Order('CHOCOLATE', choco_sell, min(4, 232-self.position['CHOCOLATE'])))
            orders['ROSES'].append(Order('ROSES', roses_sell, min(1, 58-self.position['ROSES'])))
            orders['STRAWBERRIES'].append(Order('STRAWBERRIES', straw_sell, min(6, 348-self.position['STRAWBERRIES'])))
        elif self.side == 'rebalance':
